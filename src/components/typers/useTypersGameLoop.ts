...
import { ASTEROID_PATHS, IMPACT_DURATION_MS } from "#/lib/typers-visuals";
import type { TFallingWord, TImpact } from "#/types/typers";
...
export function useTypersGameLoop({
	...
	const impactsRef = useRef<TImpact[]>([]);
	...
	useEffect(() => {
		...
		const step = () => {
			...
			impactsRef.current = impactsRef.current.filter(
				(imp) => now - imp.startedAt < IMPACT_DURATION_MS,
			);
			...
			if (wordsRef.current.length < numWordsToSpawn) {
				...
				if (pos !== null) {
					wordsRef.current.push({
						...
						variant: Math.floor(Math.random() * ASTEROID_PATHS.length),
					});
				}
			}
